...
final class AudioViewController: UICollectionViewController, UISearchBarDelegate{
    ...
    func loadProducts() {
        ...
        if (isWordpress) {
            ...
            SoundCloudAPI.sharedInstance()!.soundCloudSongs(params![0], type: params![1], offset: (page - 1) * perPage, limit: perPage, completionHandler: { resultArray, error in
                ...
                if (resultArray == nil) {
                    ...
                    self.collectionView?.reloadData()
                    self.handleResults(cantLoadMore: (resultArray?.count ?? 0) < 10)

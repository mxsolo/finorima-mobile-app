...
final class SocialViewController: HideableCollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        if ((params[0]).range(of: "http") != nil && (params[0]).range(of: "wp-json/wp/v2/") == nil){
    ...
    func loadPosts() {
        ...
        if ((self.provider == 1 || self.provider == 2) && (params.count < 2 || params[1].isEmpty)) {
            let snack = LPSnackbar(title:"No API key in config.json!", buttonTitle: "OK")
        ...
        if (self.provider == 1) {
            ...
            apiKey = AppDelegate.PINTEREST_ACCESS_TOKEN
        } else if (self.provider == 4){
            provider = TwitterProvider.init()
        }
        
        client?.get(identifier: params[0], apiKey: apiKey, requestParams: requestParams, provider: provider!, completionHandler: { (success, posts, pageToken) in
            if (!success) {

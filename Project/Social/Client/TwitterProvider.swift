...
class TwitterProvider: SocialProvider {
    ...
    public func get(identifier: String, requestParams: SocialRequestParams, completionHandler: @escaping (Bool, [SocialItem]?, String?) -> Void) {
        ...
        if let pageToken = requestParams.nextPageToken {
            requestParams.nextPageToken = String(Int(pageToken)! - 1)
        }
        
        if (identifier.starts(with: "?")) {
            let query = String(format: "%@ -filter:retweets",identifier.replacingOccurrences(of: "?", with: ""))
            
            swifter.searchTweet(using: query, geocode: nil, lang: nil, locale: nil, resultType: "mixed", count: 20, until: nil, sinceID: nil, maxID: requestParams.nextPageToken, includeEntities: true, callback: nil, tweetMode: TweetMode.extended, success: { (json, meta) in
            ...
            swifter.getTimeline(for: .screenName(identifier), customParam: ["screen_name":identifier], count: 20, sinceID: nil, maxID: requestParams.nextPageToken, trimUser: false, excludeReplies: true, includeRetweets: false, contributorDetails: true, includeEntities: true, tweetMode: TweetMode.extended, success: { (json) in

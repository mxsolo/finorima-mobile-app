...
final class WordpressSwiftViewController: HideableCollectionViewController, UISearchBarDelegate{
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (identifierForPath(indexPath: indexPath) == PostCategorySlider.identifierX){
            ...
            if (shouldShowCategories()){
                categorySlider?.baseUrl = self.params[0]
    ...
    func shouldShowCategories() -> Bool {
        ...
        return params.count == 1 || (params[1]).isEmpty
    }
    ...
    func loadProducts() {
        ...
        client?.get(blogURL: params![0] , params: requestParams, forType: WPPost.self, completionHandler: { (success, posts) in
            if (!success) {
